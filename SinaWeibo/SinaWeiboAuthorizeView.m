...
- (void)sizeToFitOrientation:(BOOL)transform
{
    ...
    CGFloat scaleFactor = SinaWeiboIsDeviceIPad() ? 1.0f : 1.0f;
    ...
    self.center = center;
    closeButton.frame = CGRectMake(self.frame.size.width-25, 0, 25, 25);
...
//    if (SinaWeiboIsDeviceIPad()) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            NSString *string = [NSString stringWithContentsOfURL:[NSURL URLWithString:authPagePath] encoding:NSUTF8StringEncoding error:nil];
            if (SinaWeiboIsDeviceIPad()) {
                 string  = [string stringByReplacingOccurrencesOfString:@"width=device-width" withString:@"width=440"];
            }else{
                 string  = [string stringByReplacingOccurrencesOfString:@"width=device-width" withString:@"width=300"];
            }
            dispatch_async(dispatch_get_main_queue(), ^{
                [self hideIndicator];
                if (string) {
                    [webView loadHTMLString:string baseURL:[NSURL URLWithString:authPagePath]];
                }
            });
        });
//    }
...
//    }
    ...
    webView.frame = CGRectMake(kBorderWidth, kBorderWidth, innerWidth,
...
//    [modalBackgroundView removeFromSuperview];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"Weibo_AuthorizeView_Hide" object:nil];
}
