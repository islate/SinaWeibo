...
@interface SinaWeiboAuthorizeViewNaviController : UINavigationController

@end

@implementation SinaWeiboAuthorizeViewNaviController
...
@property (nonatomic,retain) SinaWeiboAuthorizeView *authorizeView;
...
- (void)viewDidLoad
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(done) name:@"Weibo_AuthorizeView_Hide" object:nil];
    ...
    if (authorizeView) {
        authorizeView.frame = self.view.bounds;
...
- (void)logIn
{
    ...
    else
    {
        ...
        if ([device respondsToSelector:@selector(isMultitaskingSupported)] &&
            [device isMultitaskingSupported] && self,useSSO)
        {
        ...
        if (!ssoLoggingIn)
        {
            ...
            SinaWeiboAuthorizeViewController *controller = [[SinaWeiboAuthorizeViewController alloc] init];
            controller.authorizeView = authorizeView;
            
            SinaWeiboAuthorizeViewNaviController *naviController = [[SinaWeiboAuthorizeViewNaviController alloc] initWithRootViewController:controller];
            ...
            // linyize fix bbwc #255
            if ([presentingViewController isKindOfClass:[SinaWeiboAuthorizeViewNaviController class]])
            {
                return;
            }
            ...
            @try {
                [presentingViewController presentViewController:naviController animated:YES completion:nil];
            }
...
- (BOOL)handleOpenURL:(NSURL *)url
{
    ...
    if ([urlString hasPrefix:self.ssoCallbackScheme])
    {
        ...
        else
        {
            ...
            //modified by mengxiangjian
            if ([SinaWeiboRequest getParamValueFromUrl:urlString paramName:@"sso_error_user_cancelled"] || [SinaWeiboRequest getParamValueFromUrl:urlString paramName:@"failed"])
            {
                if ([delegate respondsToSelector:@selector(sinaweiboLogInDidCancel:)])
                {
                    [delegate sinaweiboLogInDidCancel:self];
